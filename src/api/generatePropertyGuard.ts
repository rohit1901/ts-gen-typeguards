import {isPropertySignature, isTypeLiteralNode, TypeElement} from 'typescript';
 ...
 * @param parentName
 */
export function generatePropertyGuard(property: TypeElement, parentName?: string) {
  ...
  if (parentName) {
    typeGuard.push(`value.${parentName}.hasOwnProperty('${property.name.getText()}')`);
  } else {
    typeGuard.push(`value.hasOwnProperty('${propertyName}')`);
  }
  if(property.questionToken) return generateOptionalPropertyTypeGuard(property, parentName);
  if(isTypeLiteralNode(property.type)) typeGuard.push(...generateTypeLiteralTypeGuard(property.type, propertyName));
