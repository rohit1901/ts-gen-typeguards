...
export function generateOptionalPropertyTypeGuard(
    ...
    if (isLiteralTypeNode(type)) {
        typeGuardCode.push(createTypeguardString(name.getText(), `value.${name.getText()} === ${type.getText()}`, false))
    } else if (isTypeReferenceNode(type)) {
        typeGuardCode.push(createTypeguardString(name.getText(), capitalize(type.getText()), true))
        ...
        typeGuardCode.push(createTypeguardString(name.getText(), `typeof value.${name.getText()} === '${type.getText()}'`, false))
...
function createTypeguardString(propertyName: string, text: string, isTypeReference?: boolean) {
  if(isTypeReference) return `(typeof value.${propertyName} === 'undefined' || is${text}(value.${propertyName}))`
