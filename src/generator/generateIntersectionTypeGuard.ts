...
export function generateIntersectionTypeGuard(
    ...
    for (const intersectionType of type.types) {
        if (isTypeReferenceNode(intersectionType)) {
            generateTypeReferenceTypeGuards(intersectionType, typeAliases, encounteredPropertyNames, typeGuardCode);
        } else if (isTypeLiteralNode(intersectionType)) {
            generateTypeLiteralGuards(intersectionType, encounteredPropertyNames, typeGuardCode, typeAliases);
        }
    }
...
function generateTypeReferenceTypeGuards(
    ...
    const typeLiterals = typeAliases.filter(
        (typeAlias) => typeAlias.name.getText() === intersectionType.typeName.getText()
    );

    for (const literals of typeLiterals) {
        if (isTypeLiteralNode(literals.type)) {
            generateTypeLiteralGuards(literals.type, encounteredPropertyNames, typeGuardCode, typeAliases);
        }
    }
}
/**
    ...
    for (const member of typeLiteral.members) {
        if (isPropertySignature(member)) {
            if (isIntersectionTypeNode(member.type)) {
                generateIntersectionTypeGuards(member.type, encounteredPropertyNames, typeGuardCode, typeAliases);
            } else {
...
function generateIntersectionTypeGuards(
    ...
    for (const intersectionMember of intersectionType.types) {
        if (isTypeReferenceNode(intersectionMember)) {
            ...
            generateTypeReferenceTypeGuards(intersectionMember, typeAliases, encounteredPropertyNames, typeGuardCode);
        } else if (isTypeLiteralNode(intersectionMember)) {
            // TODO: handle intersection type literal
            generateTypeLiteralGuards(intersectionMember, encounteredPropertyNames, typeGuardCode, typeAliases);
        } else {
